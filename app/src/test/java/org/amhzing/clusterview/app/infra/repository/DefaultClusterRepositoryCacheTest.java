...
import static org.amhzing.clusterview.app.infra.repository.DefaultGroupRepositoryCacheTest.CACHE_TEST_KEY;
...
public class DefaultClusterRepositoryCacheTest {
    ...
    public void should_evict_caches_when_saving_course_stats() throws Exception {
        ...
        assertThat(cacheValue(STATS_COURSE_CACHE_NAME, CACHE_TEST_KEY)).isNull();
    }

    private void givenAPopulatedCache() {
        given(clusterJpaRepository.findOne(EXISTING)).willReturn(clusterEntity());
    ...
    private void whenSavingCourseStats() {
        when(clusterJpaRepository.save(any(ClusterEntity.class))).thenReturn(clusterEntity());
