...
public class DefaultGroupRepositoryCacheTest {
    ...
    public void should_invoke_group_cache() throws Exception {
        ...
        given(teamJpaRepository.findOne(EXISTING_GROUP)).willReturn(JpaRepositoryHelper.teamEntity());
    ...
    private void givenAPopulatedCache() {
        given(clusterJpaRepository.findOne(EXISTING_CLUSTER)).willReturn(JpaRepositoryHelper.clusterEntity());
        given(clusterJpaRepository.findOne(EXISTING_CLUSTER_2)).willReturn(JpaRepositoryHelper.clusterEntity());
        given(teamJpaRepository.findOne(EXISTING_GROUP)).willReturn(JpaRepositoryHelper.teamEntity());
        given(teamJpaRepository.findOne(EXISTING_GROUP_2)).willReturn(JpaRepositoryHelper.teamEntity());
    ...
    private void whenCreatingGroup() {
        when(teamJpaRepository.save(any(TeamEntity.class))).thenReturn(JpaRepositoryHelper.teamEntity());
